...
import { getVideosDetail } from '../api';
...
componentDidMount() {
  ...
  getVideosDetail({idVideo: match.params.id})
  ...
  render() {
    ...
    const { match } = this.props;
    if(error) return (<p className="error loader">{error.message}</p>);
    if(isLoading ||!video) return (<Loading message={`Cargado video # ${match.params.id}...`}/>);
