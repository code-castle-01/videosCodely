const FAKE_DELAY = 2000; // 2 segundos
const errorVideo = "No se ha encontrado el video ;(";
const FAKE_DATA = [ 
	{	
		id:0,
...
export const getVideosDetail = ({idVideo}) => new Promise((resolve, reject) => {
	setTimeout(() => {
		const video = FAKE_DATA.find((arrayElement) => parseInt(arrayElement.id) === parseInt(idVideo));
		// Somethinng goes wrong
		if (!video) return reject({message:errorVideo});
		// All is ok
